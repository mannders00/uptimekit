...
cleanup() { docker rm -f "$NAME" >/dev/null 2>&1 || true; docker network rm "$NAME" >/dev/null 2>&1 || true; }
trap cleanup EXIT
...
printf '{"environment":"%s","restore_seconds":%s,"verified_at":"%s"}\n' "$ENVIRONMENT" "$DURATION" "$(date -u +%FT%TZ)" | tee "$STATE/restore.json"
